    ...
    def __init__(
        ...
        **kwargs,
        ...
        for idx, frame in enumerate(frames):
            image_path = frame['file_path'].replace('samples-rgb', 'images')
        ...
        for index in range(0, len(image_names), 10):
            colors = imageio.imread(image_paths[index])[..., :3]    # (H, W, 3)
            depths_path = image_paths[index].replace('images', 'depths').replace('.png', '.npy')
            depths = np.load(depths_path)[..., None]    # (H, W, 1)
            depths = torch.from_numpy(depths).float()

            c2w = camtoworlds[index]    # (4, 4)
            K = Ks_dict[index]
            c2w = torch.from_numpy(c2w).float()
            K = torch.from_numpy(K).float()

            cam_dirs = get_canonical_rays(K, (depths.shape[0], depths.shape[1]))
            points_cam = cam_dirs * depths
            points_cam = torch.cat([points_cam, torch.ones_like(points_cam[..., :1])], dim=-1)   # (h, w, 4)
            points_world = points_cam @ c2w.T
            points.append(points_world[..., :3].cpu().numpy().reshape(-1, 3))
            points_rgb.append(colors.reshape(-1, 3))
        ...
        pc.colors = points_rgb
        pc.export("points.ply", "ply")
        exit()
