...
def exp_tonemap(hdr_image):
    image_Yxy = convert_rgb_2_Yxy(hdr_image)
    lum = (
        hdr_image[:, :, 0:1] * 0.2125
        + hdr_image[:, :, 1:2] * 0.7154
        + hdr_image[:, :, 2:3] * 0.0721
    )
    lum = torch.log(torch.clamp(lum, min=1e-6))
    lum_mean = torch.exp(torch.mean(lum))
    lp = image_Yxy[:, :, 0:1] * 0.18 / torch.clamp(lum_mean, min=1e-6)
...
if __name__ == "__main__":
    ...
    hdr_image = torch.rand(128, 128, 3) * 6

    ldr_image = tonemap(hdr_image)

    restored_hdr_image = inverse_tonemap(ldr_image)

    diff = hdr_image - restored_hdr_image

    # save to png
    print(diff.max(), diff.mean())
    
    diff = ((diff / 6).numpy() ** 255).astype(np.uint8)
