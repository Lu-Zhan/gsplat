...
class Runner:
    ...
    def __init__(self, cfg) -> None:
        ...
        if not self.cfg.disable_viewer:
            ...
            if self.cfg.view_model == 'color':
                self.viewer = nerfview.Viewer(
                    server=self.server,
                    render_fn=self._viewer_render_fn,
                    mode="training",
                )
            elif self.cfg.view_model == 'intrinsics':
                self.viewer = nerfview.Viewer(
                    server=self.server,
                    render_fn=self._viewer_render_intrinsics_fn,
                    mode="training",
                )
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            if step % 2000 == 0:
                ...
                wandb.log({
                    'train/vol': wandb.Image(vol_image), 
            ...
            if (cfg.irradiance_loss or cfg.surface_rendering_loss) and step >= cfg.surface_rendering_start_iter:
                pbr_result = render_reflection(
                    ...
                    hw=(height, width),
                    camtoworlds=camtoworlds,
                    depth_map=depths.data,
                    normal_map=normals.data,
    ...
    @torch.no_grad()
    def _viewer_render_intrinsics_fn(
        self, camera_state: nerfview.CameraState, img_wh: Tuple[int, int]
    ):
        """Callable function for the viewer."""
        W, H = img_wh
        ...
        render_colors = render_colors[0, ..., 3:6].cpu().numpy()
